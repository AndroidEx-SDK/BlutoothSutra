...
public class StringUtil1 {
    ...
    public static String parseStr(String value) {
        if (StringUtil1.isNullString(value)) return "0.0";
    ...
    public static int getStringLength(final String strSource) {
        ...
        for (char arrChar : arrChars) {
            iSrcLen += StringUtil1.getCharLength(arrChar);
        }
        return iSrcLen;
    }
    ...
    public static String sub(final String strSource, final int iSubLen,
        ...
        if (StringUtil1.isNull(strSource)) {
            return strSource;
        }
        String strFilter = strSource.trim(); // 过滤首尾空字符
        int iLength = StringUtil1.getStringLength(strFilter); // 字符的长度
        if (iLength <= iSubLen) {
        ...
        iNum -= StringUtil1.getCharLength(c);
        ...
        if (!StringUtil1.isNull(strSuffix)) {
            strFilter += strSuffix;
        }
        return strFilter;
    }
    ...
    public static String subWithDots(final String strSource, final int iSubLen) {
        return StringUtil1.sub(strSource, iSubLen, "...");
    }
