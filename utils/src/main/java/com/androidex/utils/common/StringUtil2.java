package com.androidex.utils.common;

/**
 * Created by cts on 17/5/8.
 */
...
public class StringUtil2 {
    ...
    public static String parseStr(String value) {
        if (StringUtil2.isNullString(value)) return "0.0";
    ...
    public static String parseStr2(String value) {
        if (StringUtil2.isNullString(value)) return "--";
        DecimalFormat df = new DecimalFormat("######0.0");
        double mvalue = Double.parseDouble(value);
    ...
    public static int getStringLength(final String strSource) {
        ...
        for (char arrChar : arrChars) {
            iSrcLen += StringUtil2.getCharLength(arrChar);
        }
        return iSrcLen;
    }
    ...
    public static String sub(final String strSource, final int iSubLen,
        ...
        if (StringUtil2.isNull(strSource)) {
            return strSource;
        }
        String strFilter = strSource.trim(); // 过滤首尾空字符
        int iLength = StringUtil2.getStringLength(strFilter); // 字符的长度
        if (iLength <= iSubLen) {
        ...
        iNum -= StringUtil2.getCharLength(c);
        ...
        if (!StringUtil2.isNull(strSuffix)) {
            strFilter += strSuffix;
        }
        return strFilter;
    }
    ...
    public static String subWithDots(final String strSource, final int iSubLen) {
        return StringUtil2.sub(strSource, iSubLen, "...");
    }
